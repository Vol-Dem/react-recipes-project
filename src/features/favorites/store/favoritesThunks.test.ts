...
const createStore = (favoriteIds: number[] = []) =>
  configureStore({
    preloadedState: {
...
describe("favorites thunks", () => {
  ...
  it("optimistically adds and persists a favorite", async () => {
    ...
    apiMocks.updateFavorite.mockResolvedValue(undefined);

    await (store.dispatch as unknown as AppDispatch)(toggleFavorite(42));
  ...
  it("ignores a duplicate update while the same recipe is pending", async () => {
    ...
    const firstUpdate = (store.dispatch as unknown as AppDispatch)(
      toggleFavorite(42),
    );
    await (store.dispatch as unknown as AppDispatch)(toggleFavorite(42));
  ...
  it("loads favorites from persistence", async () => {
    ...
    await (store.dispatch as unknown as AppDispatch)(loadFavorites("user-id"));
