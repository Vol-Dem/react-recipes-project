...
import type { RecipeState, RecipeSummary } from "../types";

const createRecipe = (id: number, calories = 0): RecipeSummary => ({
  calories,
  id,
  img: "recipe.jpg",
  readyInMinutes: 10,
...
describe("recipe selectors", () => {
  ...
  it("uses the fetched page and stored last-page flag in fallback mode", () => {
    const remoteRecipes = [createRecipe(1), createRecipe(2)];
    const remoteState = {
      recipe: {
        ...recipeState,
