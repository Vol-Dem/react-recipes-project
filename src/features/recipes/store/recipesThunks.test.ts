...
describe("recipe thunks", () => {
  ...
  describe("getRecipes", () => {
    const request = {
      ...
      firebaseRef: "firebaseRef" as never,
      filter: "filter" as never,
      position: undefined,
      resultsAmount: 10 as never,
    };
  ...
  it("changes local API pages without storing a derived collection", () => {
    ...
    nextStore.dispatch(nextPage("firebaseRef" as never, "filter" as never));
    previousStore.dispatch(prevPage("firebaseRef" as never, "filter" as never));
  ...
  it("resets the page without storing a derived sort result", () => {
    ...
    store.dispatch(sortRecipes("firebaseRef" as never, "filter" as never));
