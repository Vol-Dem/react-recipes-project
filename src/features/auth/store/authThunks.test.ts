...
const executeThunk = <Result>(
  thunk: AppThunk<Result>,
  dispatch: ReturnType<typeof vi.fn>,
) =>
  thunk(
    dispatch as unknown as AppDispatch,
    vi.fn() as unknown as () => RootState,
    undefined,
  );
...
describe("auth thunks", () => {
  ...
  it("initializes the authenticated user and their favorites", () => {
    ...
    expect(executeThunk(initAuth(), dispatch)).toBe(unsubscribe);
  ...
  it("authenticates with email and closes the form", async () => {
    ...
    await executeThunk(authRequest(true, user.email, "password"), dispatch);
  ...
  it("maps authentication failures and finishes loading", async () => {
    ...
    await executeThunk(authRequest(true, "invalid", "password"), dispatch);
  ...
  it("maps an email already in use during sign-up", async () => {
    ...
    await executeThunk(authRequest(false, user.email, "password"), dispatch);
  ...
  it("maps profile update failures", async () => {
    ...
    await executeThunk(changeUserName("Updated User"), dispatch);
    await executeThunk(changeUserPassword("New-password1"), dispatch);
  ...
  it("maps Google sign-in failures but ignores popup cancellation", async () => {
    ...
    await executeThunk(authWithGoogle(), dispatch);
    await executeThunk(authWithGoogle(), dispatch);
  ...
  it("clears local auth state and signs out through the API", async () => {
    const dispatch = vi.fn();
    authApiMocks.signOutUser.mockResolvedValue(undefined);

    await executeThunk(logoutUser(), dispatch);
  ...
  it("reports successful password reset requests", async () => {
    const dispatch = vi.fn();
    authApiMocks.sendUserPasswordResetEmail.mockResolvedValue(undefined);

    await executeThunk(resetUserPassword(user.email), dispatch);
